...
import { IChatSessionsService, IChatSessionRequestHistoryItem } from '../../../../workbench/contrib/chat/common/chatSessionsService.js';
...
class ForkFromPromptAction extends Action2 {
	constructor() {
		super({
			id: 'sessions.promptTimeline.fork',
			title: localize2('promptTimeline.fork', "Fork Session from Prompt..."),
			category: CATEGORY,
			f1: true,
			precondition: TIMELINE_PRECONDITION,
		});
	...
	override async run(accessor: ServicesAccessor): Promise<void> {
		...
		const picked = await quickInputService.pick(items, {
			placeHolder: localize('promptTimeline.forkPlaceholder', "Pick a past prompt to fork session from"),
			matchOnDescription: true,
		});
		...
		const activeChatResource = activeSession.activeChat.get().resource;
		const turnId = picked.requestId;

		if (activeSession.capabilities.get().supportsMultipleChats) {
			const newChat = await sessionsManagementService.forkChatInSession(activeSession, activeChatResource, turnId);
			await sessionsService.openChat(activeSession, newChat.resource);
		} else {
			const chatSessionsService = accessor.get(IChatSessionsService);
			const chatWidgetService = accessor.get(IChatWidgetService);
			const widget = chatWidgetService.getWidgetBySessionResource(activeChatResource);
			if (widget && widget.viewModel) {
				const request = widget.viewModel.getItems().find(r => r.id === turnId);
				if (request && isRequestVM(request)) {
					const historyItem: IChatSessionRequestHistoryItem = {
						id: request.id,
						type: 'request',
						prompt: request.messageText,
						participant: request.username
					};
					await chatSessionsService.forkChatSession(activeChatResource, historyItem, new CancellationTokenSource().token);
				}
			}
		}
	}
...
export function registerPromptTimelineActions(): void {
	...
	registerAction2(ForkFromPromptAction);
}
