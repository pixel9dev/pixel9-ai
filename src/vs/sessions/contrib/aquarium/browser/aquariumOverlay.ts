...
export const IAquariumService = createDecorator<IAquariumService>('aquariumService');
	...
	export interface IAquariumService {
		...
		/**
		 * Feed the aquarium fish by spawning food pellets and updating the feeding streak.
		 */
		feedFish(count?: number): void;

		/**
		 * Update water quality based on code quality (crystal, murky, toxic)
		 */
		updateWaterQuality(quality: 'crystal' | 'murky' | 'toxic'): void;

		/**
		 * Increment the feeding streak
		 */
		incrementStreak?(): void;

		/**
		 * Reset the feeding streak
		 */
		resetStreak?(): void;
	}
...
export class AquariumService extends Disposable implements IAquariumService {
	...
	feedFish(count: number = 1): void {
		if (this.activeRef.value && this.activeRef.value.spawnFood) {
			const water = this.mainContainer.querySelector('.agents-aquarium-water');
			const width = water ? water.clientWidth : 500;
			const height = water ? water.clientHeight : 200;
			for (let i = 0; i < count; i++) {
				const dropX = Math.random() * width;
				const dropY = 10 + Math.random() * (height / 3);
				this.activeRef.value.spawnFood(dropX, dropY);
			}
		}
		this.handleFishFed();
	}
	...
	/**
	 * Update water quality based on code audit results
	 * crystal: all code passes audits
	 * murky: dead code or anomalies detected
	 * toxic: unresolved dependencies or violations
	 */
	updateWaterQuality(quality: 'crystal' | 'murky' | 'toxic'): void {
		if (this.activeRef.value) {
			const water = this.mainContainer.querySelector('.agents-aquarium-water');
			if (water) {
				water.classList.remove('crystal', 'murky', 'toxic');
				water.classList.add(quality);
				// Apply CSS filter for visual effect
				switch (quality) {
					case 'crystal':
						(water as HTMLElement).style.filter = 'brightness(1.2) saturate(1.5)';
						break;
					case 'murky':
						(water as HTMLElement).style.filter = 'brightness(0.8) saturate(0.7) hue-rotate(30deg)';
						break;
					case 'toxic':
						(water as HTMLElement).style.filter = 'brightness(0.6) saturate(0.5) hue-rotate(350deg)';
						break;
				}
			}
		}
	 ...
	 */
	resetStreak(): void {
		// Reset streak by clearing storage
		this.streak.simulate(0, true);
		this.updateAllToggleButtonsVisual(!!this.activeRef.value);
	}
}
...
interface IActiveAquarium extends IDisposable {
	...
	spawnFood?(dropX: number, dropY: number): void;
}
