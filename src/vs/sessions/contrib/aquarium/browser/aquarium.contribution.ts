...
import './aquarium-agent-integration.js';
...
class FeedAquariumFishAction extends Action2 {

	static readonly ID = 'sessions.aquarium.feed';

	constructor() {
		super({
			id: FeedAquariumFishAction.ID,
			title: localize2('aquarium.feed', "Feed Aquarium Fish"),
			f1: true,
			category: Categories.Developer,
		});
	}

	override async run(accessor: ServicesAccessor, count?: number): Promise<void> {
		const aquariumService = accessor.get(IAquariumService);
		aquariumService.feedFish(typeof count === 'number' ? count : 3);
	}
}

registerAction2(FeedAquariumFishAction);
